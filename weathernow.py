...
def ShowWeatherNow(data: dict) -> None:
    ...
    now_city = to_city_dt(data["dt"], data["timezone"]).strftime("%H:%M")
    ...
    c7.metric("Clouds (%)", str((data.get("clouds") or {}).get("all")) if (data.get("clouds") or {}).get("all") is not None else "N/A")

    sunrise = to_city_dt(sys["sunrise"], data["timezone"]).strftime("%H:%M")
    sunset = to_city_dt(sys["sunset"], data["timezone"]).strftime("%H:%M")
